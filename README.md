...
Measured on a V100 32GB at 1120×768, 124 frames: **~30 s/step vs 330–370
s/step fp32 (~11×)**. Output is indistinguishable from the fp32 path.

## Why fp16 breaks

Three separate places in H3 genuinely exceed fp16's maximum representable
value (65504):
...
2. **The residual stream** — attention-sink rows grow from ~81k to ~4.3M
   across the 50 DiT blocks. Because the magnitude grows ~50× through the
...
2. **The residual stream stays fp32** across all 50 blocks; the
   ...
   in fp32. The matmuls are ~99% of the compute — that's where the speed
   lives.
